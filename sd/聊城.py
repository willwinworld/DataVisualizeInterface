...
def tpxw(url):  # 图片新闻
    ...
    first_page_table_urls = remove_duplicate_urls([i('a').attr('href') for i in
                                                   d(
                                                       '#newslistComponentlmgcahbgaciobboelpaelbcbedofgphc td[style]').items()
                                                   if
    ...
    contain_page_num_url = d('#newslistComponentlmgcahbgaciobboelpaelbcbedofgphc tr').eq(-1).find('a').eq(-1).attr(
        'href')
    ...
    for idx in xrange(2, total_page_num + 1):
        logger.info(idx)
        form_data = {'goPage': 1, 'pageNum': idx, 'siteID': 'lcsrmjcy', 'pageID': 'dnhdfodlaciobboelpaelbcbedofgphc',
                     'moduleID': 'lmgcahbgaciobboelpaelbcbedofgphc',
                     'moreURI': '/ecdomain/framework/lcsrmjcy/dnhdfodlaciobboelpaelbcbedofgphc/lmgcahbgaciobboelpaelbcbedofgphc.do',
                     'var_temp': 'cpjjfhjdablobboelmcdpijjdaflpnal',
                     'showChildFlag': 'false',
        ...
        other_page_table_urls = remove_duplicate_urls([i('a').attr('href') for i in post_d(
            '#newslistComponentlmgcahbgaciobboelpaelbcbedofgphc td[style]').items() if
                                                       i('a').attr('href') is not None])
...
def other(url):  # 其它所有模块
    r = requests.get(url)
    r.encoding = 'utf-8'
    d = Pq(r.content).make_links_absolute(base_url=url)
    block_urls = remove_duplicate_urls([i('.ALink').attr('href') for i in d('table').items()])
    return block_urls
...
if __name__ == '__main__':
    # tpxw('http://10.137.140.57:8080/ecdomain/framework/lcsrmjcy/dnhdfodlaciobboelpaelbcbedofgphc.jsp')
    # other('http://10.137.140.57:8080/ecdomain/framework/lcsrmjcy/index.htm')

    try:
        tpxw_news_list = remove_duplicate_urls(tpxw('http://10.137.140.57:8080/ecdomain/framework/lcsrmjcy/dnhdfodlaciobboelpaelbcbedofgphc.jsp'))
    ...
    try:
        other_news_list = remove_duplicate_urls(other('http://10.137.140.57:8080/ecdomain/framework/lcsrmjcy/index.htm'))
    ...
    requests.post('http://192.168.1.190:18080/COQuery/pachong', data={'belong': 'lcsy'})